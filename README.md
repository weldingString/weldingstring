...
The project is a multiuser project and start with /start/userid+tennant and stop by /logout/userid+tennan 
All images and weld string are generated under that actual userid. If you start it without this commando
you will get standard userID for all users. It is meaning that this userid is the same userid as used in Weldit system
...
System sending an API to https://tennant.weldit.no/api/images/userid  This URL is given in method sendSvgToApi()
...
type `mvn` (Windows), or `./mvnw` (Mac & Linux), then open
...
To create a production build, call `mvn java clean package -Pproduction` (Windows),
